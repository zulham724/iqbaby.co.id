...
if ( ! class_exists( 'YITH_WCAN_Stock_On_Sale_Widget' ) ) {
	...
	class YITH_WCAN_Stock_On_Sale_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			parent::__construct( $this->_id_base, __( 'YITH AJAX In Stock/On Sale Filters', 'yith-woocommerce-ajax-navigation' ), $widget_ops, $control_ops );
		...
		public function stock_on_sale_filter_args( $link ) {
			if ( ! empty( $_GET['onsale_filter'] ) ) {
				$link = add_query_arg( array( 'onsale_filter' => (int) $_GET['onsale_filter'] ), $link );
			}

			if ( ! empty( $_GET['instock_filter'] ) ) {
				$link = add_query_arg( array( 'instock_filter' => (int) $_GET['instock_filter'] ), $link );
			}
