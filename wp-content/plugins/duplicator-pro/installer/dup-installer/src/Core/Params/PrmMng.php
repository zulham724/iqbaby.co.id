...
class PrmMng
{
    ...
    const PARAM_SUBSITE_OVERWRITE_ID                   = 'subsite_owr_id';
    const PARAM_SUBSITE_OVERWRITE_NEW_SLUG             = 'subsit_owr_slug';
    ...
    const PARAM_CONTENT_OWNER                          = 'cont_owner';
    ...
    protected function setParamsValues($arrayData, $overwrite = false, $logginLevelSet = Log::LV_DEBUG)
    {
        ...
        foreach ($arrayData as $key => $arrayValues) {
            if (isset($this->params[$key])) {
                ...
                $this->paramsHtmlInfo[] = 'SET PARAM <b>' . $key . '</b> ARRAY DATA: ' . SnapString::implodeKeyVals(', ', $arrayValValToStr, '[<b>%s</b> = %s]');
