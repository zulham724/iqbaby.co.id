...
class ParamItem
{
    ...
    public function setValueFromInput($method = self::INPUT_POST)
    {
        ...
        Log::info('SET VALUE FROM INPUT KEY [' . $this->name . '] VALUE[' . Log::v2str(isset($superObject[$this->name]) ? $superObject[$this->name] : '') . ']', Log::LV_DEBUG);
    ...
    protected function isValidScalar($value, &$validateValue = null)
    {
        ...
        switch ($this->type) {
            ...
            case self::TYPE_ARRAY_STRING:
                ...
                if (strlen($validateValue) < $this->attr['min_len'] || ($this->attr['max_len'] > 0 && strlen($validateValue) > $this->attr['max_len'])) {
                    $result = false;
                }
    ...
    protected function callValidateCallback($value)
    {
        if (is_callable($this->attr['validateCallback'])) {
            return call_user_func($this->attr['validateCallback'], $value);
