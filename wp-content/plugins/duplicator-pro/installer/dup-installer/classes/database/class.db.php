...
class DUPX_DB
{
    ...
    public static function connect($host, $username, $password, $dbname = null)
    {
        try {
            ...
            $dbh = @mysqli_connect($host, $username, $password, null, $port, $socket);
            if (!$dbh) {
...
// First peel off the socket parameter from the right, if it exists.
    ...
    public static function renameTable($dbh, $existing_name, $new_name, $delete_if_conflict = false)
    {
        ...
        Log::info('RENAME TABLE ' . $existing_name . ' TO ' . $new_name, Log::LV_DETAILED);
    ...
    public static function copyTable($dbh, $existing_name, $new_name, $delete_if_conflict = false)
    {
        ...
        Log::info('COPY TABLE ' . $existing_name . ' TO ' . $new_name, Log::LV_DETAILED);
        $escapedOldName = mysqli_real_escape_string($dbh, $existing_name);
        $escapedNewName = mysqli_real_escape_string($dbh, $new_name);
     ...
     */
    protected static function chunksAction($dbh, $action, $table, $where)
    {

        $sql = $action . ' FROM ' . mysqli_real_escape_string($dbh, $table) . ' WHERE ' . $where . ' LIMIT ' . self::DELETE_CHUNK_SIZE;
    ...
    {
        return self::chunksAction($dbh, 'DELETE', $table, $where);
    }
     ...
     * @return int affected rows
     */
    public static function chunksUpdate($dbh, $table, $where)
    {
        return self::chunksAction($dbh, 'UPDATE', $table, $where);
    }
}
