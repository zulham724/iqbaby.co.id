...
MSG;
    ...
    public static function updateTablePrefix($dbh, $table, $column, $oldPrefix, $newPrefix)
    {
        ...
        $regexOldTablePrefix   = mysqli_real_escape_string($dbh, preg_quote($oldPrefix, null /* no delimiter */));
    ...
    {
        if (
            !DUPX_ArchiveConfig::getInstance()->isTablePrefixChanged() &&
        ...
        if (
            ...
            $oldPrefix  = $subsiteObj->blog_prefix;
        } else {
            $oldPrefix = DUPX_ArchiveConfig::getInstance()->wp_tableprefix;
        }

        if (DUPX_InstallerState::isAddSiteOnMultisite()) {
            $newPrefix    = DUPX_MU::getSubsiteOverwriteTablePrefix();
            $optionsTable = mysqli_real_escape_string($dbh, DUPX_DB_Functions::getOptionsTableName(DUPX_MU::getSubsiteOverwriteTablePrefix()));
        } else {
        ...
        if (!DUPX_InstallerState::isAddSiteOnMultisite() && in_array($usermetaTable, $tables)) {
