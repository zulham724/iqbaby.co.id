...
<p>
    <?php if (!$dbsOfSameType) { ?>
        Some versions of different database engines are not compatible with each other, which might cause problems with the database import. We suggest continuing the install as usual.
        In case there are problems with the database install please consult the links in the troubleshoot section to find out which features are not compatible.
    <?php } else { ?>
        The source site used a newer version of <?php echo htmlentities($hostDBEngine); ?>, which may result in problems during the installation if there were changes
        made which are not backward-compatible. We suggest continuing with the install as usual. In case of problems with the database try contacting your hosting provider and asking for a database version upgrade.
    <?php } ?>
</p>
