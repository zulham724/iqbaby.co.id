...
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/storage/class.storage.supported.php');
...
require_once(DUPLICATOR_PRO_PLUGIN_PATH . '/classes/net/class.ftp.curl.php');
...
class DUP_PRO_Storage_Entity extends DUP_PRO_JSON_Entity_Base
{
    ...
    public function encrypt()
    {
        ...
        if (!empty($this->ftp_username)) {
            $this->ftp_username = DUP_PRO_Crypt_Blowfish::encrypt($this->ftp_username);
        }

        if (!empty($this->ftp_password)) {
            $this->ftp_password = DUP_PRO_Crypt_Blowfish::encrypt($this->ftp_password);
        }

        if (!empty($this->ftp_storage_folder)) {
            $this->ftp_storage_folder = DUP_PRO_Crypt_Blowfish::encrypt($this->ftp_storage_folder);
        }

        if (!empty($this->sftp_username)) {
            $this->sftp_username = DUP_PRO_Crypt_Blowfish::encrypt($this->sftp_username);
        }

        if (!empty($this->sftp_password)) {
            $this->sftp_password = DUP_PRO_Crypt_Blowfish::encrypt($this->sftp_password);
        }

        if (!empty($this->sftp_private_key)) {
            $this->sftp_private_key = DUP_PRO_Crypt_Blowfish::encrypt($this->sftp_private_key);
        }

        if (!empty($this->sftp_private_key_password)) {
            $this->sftp_private_key_password = DUP_PRO_Crypt_Blowfish::encrypt($this->sftp_private_key_password);
        }

        if (!empty($this->sftp_storage_folder)) {
            $this->sftp_storage_folder = DUP_PRO_Crypt_Blowfish::encrypt($this->sftp_storage_folder);
        }

        if (!empty($this->onedrive_user_id)) {
            $this->onedrive_user_id = DUP_PRO_Crypt_Blowfish::encrypt($this->onedrive_user_id);
        }
    ...
    public function decrypt()
    {
        ...
        if ($this->version != '0.0.0.0') {
            ...
            if (!empty($this->ftp_username)) {
                $this->ftp_username = DUP_PRO_Crypt_Blowfish::decrypt($this->ftp_username);
            }

            if (!empty($this->ftp_password)) {
                $this->ftp_password = DUP_PRO_Crypt_Blowfish::decrypt($this->ftp_password);
            }

            if (!empty($this->ftp_storage_folder)) {
                $this->ftp_storage_folder = DUP_PRO_Crypt_Blowfish::decrypt($this->ftp_storage_folder);
            }

            if (!empty($this->sftp_username)) {
                $this->sftp_username = DUP_PRO_Crypt_Blowfish::decrypt($this->sftp_username);
            }

            if (!empty($this->sftp_password)) {
                $this->sftp_password = DUP_PRO_Crypt_Blowfish::decrypt($this->sftp_password);
            }

            if (!empty($this->sftp_private_key)) {
                $this->sftp_private_key = DUP_PRO_Crypt_Blowfish::decrypt($this->sftp_private_key);
            }

            if (!empty($this->sftp_storage_folder)) {
                $this->sftp_storage_folder = DUP_PRO_Crypt_Blowfish::decrypt($this->sftp_storage_folder);
            }

            if (!empty($this->sftp_private_key_password)) {
                $this->sftp_private_key_password = DUP_PRO_Crypt_Blowfish::decrypt($this->sftp_private_key_password);
            }

            if (!empty($this->onedrive_user_id)) {
                $this->onedrive_user_id = DUP_PRO_Crypt_Blowfish::decrypt($this->onedrive_user_id);
            }
    ...
    public function get_action_text()
    {
        ...
        switch ($this->storage_type) {
            ...
            case DUP_PRO_Storage_Types::GDrive:
                ...
                break;
            case DUP_PRO_Storage_Types::S3:
                $text = sprintf(DUP_PRO_U::__('Transferring to Amazon S3 folder:<br/> <i>%1$s</i>'), $this->get_storage_folder());
    ...
    private function copy_to_local(DUP_PRO_Package $package, DUP_PRO_Package_Upload_Info $upload_info)
    {
                ...
                if (file_exists($source_installer_filepath)) {
                    if (DUP_PRO_IO::copyFile($source_archive_filepath, $this->local_storage_folder)) {
                        ...
                        if (DUP_PRO_IO::copyFile($source_installer_filepath, $this->local_storage_folder, true)) {
                            DUP_PRO_Log::infoTrace("Successfully copied to local location $this->local_storage_folder");
                            ...
                            if (DUP_PRO_IO::copyFile($source_database_filepath, $this->local_storage_folder, true) == false) {
                                ...
                                DUP_PRO_LOG::traceError("FAIL: copy database file from $source_database_filepath to $this->local_storage_folder");
                            }

                            if (DUP_PRO_IO::copyFile($source_log_filepath, $this->local_storage_folder, true) == false) {
                                // Annoying but non-fatal
                                DUP_PRO_LOG::traceError("FAIL: copy log file from $source_log_filepath to $this->local_storage_folder");
                            }

                            if (DUP_PRO_IO::copyFile($source_scan_filepath, $this->local_storage_folder, true) == false) {
                                // Annoying but non-fatal
                                DUP_PRO_LOG::traceError("FAIL: copy scan file from $source_scan_filepath to $this->local_storage_folder");
                            }
                        ...
                        DUP_PRO_LOG::traceError("FAIL: copy from $source_archive_filepath to $this->local_storage_folder");
                        $upload_info->increase_failure_count();
                    }
                } else {
        ...
        if ($upload_info->failed) {
            DUP_PRO_Log::infoTrace('SFTP storage failed flag ($upload_info->failed) has been already set. Problem copying to $this->local_storage_folder');
        }
                ...
                try {
                    if (!$upload_info->copied_installer) {
                        ...
                        if ($dropbox->checkFileHash($installer_meta, $source_installer_filepath)) {
                            ...
                            DUP_PRO_LOG::infoTrace("**ERROR: installer upload to DropBox" . $dropbox_installer_path . ". Uploaded installer file may be corrupted. Hashes doesn't match.");
                            $upload_info->increase_failure_count();
                        }
                    } else {
                    ...
                    if (!$upload_info->copied_archive) {
                        ...
                        if ($dropbox_upload_info->error_details == null) {
                            ...
                            if ($dropbox_upload_info->file_meta != null) {
                                // Many times $dropbox_upload_info->file_meta->content_hash doesn't exist in response
                                // so put this patch to overcome the issue.
                                $is_file_content_hash_property_exists = apply_filters('duplicator_pro_dropbox_content_hash_property_exists', !property_exists($dropbox_upload_info->file_meta, 'content_hash'));
                                if (!$this->dropbox_skip_archive_validation_hash && !$is_file_content_hash_property_exists) {
                                    DUP_PRO_LOG::infoTrace('$dropbox_upload_info->file_meta->content_hash property doesn\'t exists in response, so requesting file meta again to the dropbox API server.');
                                    $dropbox_upload_info->file_meta = $dropbox->GetMetadata($dropbox_archive_path);
                                }
                                if ($this->dropbox_skip_archive_validation_hash || $dropbox->checkFileHash($dropbox_upload_info->file_meta, $source_archive_filepath)) {
                                    if ($this->dropbox_skip_archive_validation_hash) {
                                        ...
                                        DUP_PRO_LOG::infoTrace("Skipping Dropbox uploaded Archive content hash validation.");
                                    } else {
                                        DUP_PRO_LOG::infoTrace("SUCCESS: archive upload to Dropbox.");
                                    }
                                    $upload_info->copied_archive = true;
                                    ...
                                    DUP_PRO_LOG::traceError("FAIL: archive upload to Dropbox. hashes don't match! Is account out of space?");
                                    $upload_info->increase_failure_count();
                                }
                            }
                        } else {
                            DUP_PRO_LOG::traceError("FAIL: archive upload to dropbox. hashes don't match! Is account out of space? Error received from Dropbox API: $dropbox_upload_info->error_details");
                    ...
                    DUP_PRO_LOG::traceError("Exception caught copying package $package->Name to $this->dropbox_storage_folder. " . $e->getMessage());
                    $upload_info->increase_failure_count();
                }
            } else {
                DUP_PRO_LOG::traceError("Installer doesn't exist for $package->Name!? - $source_installer_filepath");
        ...
        if (file_exists($source_archive_filepath)) {
            if (file_exists($source_installer_filepath)) {
                ...
                try {
                    ...
                    DUP_PRO_LOG::traceError("Exception caught copying package $package->Name to $this->onedrive_storage_folder. " . $e->getMessage());
            ...
            switch ($response_code) {
                case 308:
                    ...
                    if (!empty($range) && preg_match('/bytes=0-(\d+)$/', $range, $matches)) {
                        $upload_info->archive_offset = $matches[1] + 1;
                    } else {
                        $upload_info->archive_offset = 0;
                    }
        ...
        if (file_exists($source_archive_filepath)) {
            if (file_exists($source_installer_filepath)) {
                ...
                try {
                    ...
                    DUP_PRO_LOG::traceError("Exception caught copying package $package->Name to S3 $this->s3_storage_folder: " . $e->getMessage());
                    $upload_info->increase_failure_count();
                }
            } else {
                DUP_PRO_LOG::traceError("Installer doesn't exist for $package->Name!? - $source_installer_filepath");
        ...
        if (file_exists($source_archive_filepath)) {
            if (file_exists($source_installer_filepath)) {
                try {
                    ...
                    $upload_info->failed = true;
                    $upload_info->increase_failure_count();
                    DUP_PRO_LOG::traceError("Exception caught copying package $package->Name to $this->sftp_storage_folder. " . $e->getMessage());
                }
            } else {
                DUP_PRO_LOG::traceError("Installer doesn't exist for $package->Name!? - $source_installer_filepath");
                $upload_info->failed = true;
    ...
    */
    public static function getStorageIcon($id)
    {
        $html = '';
        switch ($id) {
            case DUP_PRO_Storage_Types::Local :             $html = '<i class="fa fa-server"></i>';              break;
            case DUP_PRO_Storage_Types::Dropbox :           $html = '<i class="fab fa-dropbox"></i>';            break;
            case DUP_PRO_Storage_Types::FTP :               $html = '<i class="fas fa-network-wired"></i>';      break;
            case DUP_PRO_Storage_Types::GDrive :            $html = '<i class="fab fa-google-drive"></i>';       break;
            case DUP_PRO_Storage_Types::S3 :                $html = '<i class="fab fa-aws"></i>';                break;
            case DUP_PRO_Storage_Types::SFTP :              $html = '<i class="fas fa-network-wired"></i>';      break;
            case DUP_PRO_Storage_Types::OneDrive :
            case DUP_PRO_Storage_Types::OneDriveMSGraph :   $html = '<i class="fas fa-cloud"></i>';              break;
            default:$html = '<i class="fas fa-cloud"></i>'; break;
        }
        return $html;
    }
    ...
    public function purge_old_local_packages()
    {
        try {
            ...
            if ($this->local_max_files > 0) {
                ...
                $file_list = glob("{$this->local_storage_folder}/*");
    ...
    public function purge_old_sftp_packages()
    {
        ...
        if ($sftp) {
            ...
            if (empty($file_list)) {
                DUP_PRO_LOG::traceError("FAIL: purging SFTP packages. Problems making SFTP connection, Purging old packages not possible. Error retrieving file list for " . $this->sftp_server . ":" . $this->sftp_port . " Storage Dir: " . $this->sftp_storage_folder);
            } else {
