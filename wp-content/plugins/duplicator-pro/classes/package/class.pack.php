...
// Indicates if all storages have finished (succeeded or failed all-together)
    ...
    public function set_for_cancel()
    {
        ...
        if (!in_array($this->ID, $pending_cancellations)) {
            ...
            set_transient(DUPLICATOR_PRO_PENDING_CANCELLATION_TRANSIENT, $pending_cancellations, DUPLICATOR_PRO_PENDING_CANCELLATION_TIMEOUT);
        }
    }

    public static function get_pending_cancellations()
    {
        $pending_cancellations = get_transient(DUPLICATOR_PRO_PENDING_CANCELLATION_TRANSIENT);
    ...
    {
        if (delete_transient(DUPLICATOR_PRO_PENDING_CANCELLATION_TRANSIENT) == false) {
