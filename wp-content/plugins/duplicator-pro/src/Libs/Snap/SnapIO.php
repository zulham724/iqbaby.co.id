...
class SnapIO
{
    ...
    public static function massMove($fileSystemObjects, $destination, $exclusions = null, $exceptionOnError = true)
    {
        ...
        foreach ($fileSystemObjects as $fileSystemObject) {
            ...
            if ($shouldMove) {
                ...
                } elseif (!@rename($fileSystemObject, $newName)) {
    ...
    public static function fseek($handle, $offset, $whence = SEEK_SET)
    {
        ...
        if ($ret_val !== 0) {
            ...
            } elseif (abs($offset) > self::FILE_SIZE_LIMIT_32BIT || $filesize > self::FILE_SIZE_LIMIT_32BIT || ($offset <= 0 && ($whence == SEEK_SET || $whence == SEEK_END))) {
     ...
     * @param type $trueIfEquals // if paths are equals and is true return true else false
     * @return boolean
     */
     ...
     * @return string[]
     */
    public static function sortBySubfoldersCount($paths, $childsFirst = false, $maintainIndex = false)
    {
        $function = ($maintainIndex ? 'uasort' : 'usort');
     ...
     *
     * @param string $dir // dir with / at the end
     * @param callable $callback
     * @param array $options // array(
     *                              'regexFile'     => [bool|string|array], // if is bool alrays or never match, if is string o array of string check if rexeses match file name
     *                              'regexFolder'   => [bool|string|array], // if is bool alrays or never match, if is string o array of string check if rexeses match file name
    ...
    protected static function regexGlobCallbackPrivate($dir, $callback, &$options)
    {
        ...
        while (($elem = readdir($dh)) !== false) {
            ...
            $regex    = $isDir ? $options['regexFolder'] : $options['regexFile'];
    ...
    public static function getContentsOfPath($path)
    {
        if (!is_dir($path)) {
            return array();
        }
