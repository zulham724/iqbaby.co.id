...
function _duplicatorGetRootPath()
{
        ...
        $root  = duplicator_pro_get_home_path();
            ...
            <div class="container">
                <div class="data">
                    {{#if ARC.Status.HasFilteredSites}}
                        <p class="red">
                            <?php
                            DUP_PRO_U::esc_html_e("Some sites have been excluded from the network. With this package it will not be possible to restore the network but only perform subsite to standalone conversions if you have a Business or Gold license.");
                            ?>
                        </p>
                    ...
                    <p class="red">
                        <?php
                        DUP_PRO_U::esc_html_e("Tables and/or paths have been manually excluded from some sites so the package will not be compatible with the Drag and Drop import. An install using the installer.php can still be performed, however.");
                        ?>
                    </p>
                    {{#each ARC.Subsites as |site|}}
                        {{#compare site.filtered_tables.length '||' site.filtered_paths.length}}
                            <p><b>{{site.blogname}}</b></p>
                            <div class="subsite-filter-info">
                                {{#compare site.filtered_tables.length '>' 0}}
                                    <?php DUP_PRO_U::esc_html_e('Tables:'); ?>
                                    <ol>
                                        {{#each site.filtered_tables as |filteredTable|}}
                                        <li>{{filteredTable}}</li>
                                        {{/each}}
                                    </ol>
                                {{/compare}}
                                {{#compare site.filtered_paths.length '>' 0}}
                                <?php DUP_PRO_U::esc_html_e('Paths:'); ?>
                                <ol>
                                    {{#each site.filtered_paths as |filteredPath|}}
                                    <li>{{filteredPath}}</li>
                                    {{/each}}
                                </ol>
                                {{/compare}}
...
<div id="migratepackage-block" class="scan-item scan-item-last">
    <div class='title' onclick="DupPro.Pack.toggleScanItem(this);">
        <div class="text"><i class="fa fa-caret-right"></i> <?php DUP_PRO_U::esc_html_e('Migration Status');?></div>
            ...
            <div class="container">
                <div class="data">
                    {{#if ARC.Status.CanbeMigratePackage}}
                        <?php DUP_PRO_U::esc_html_e("Packages created on this server can be migrated to different servers."); ?>
                    {{else}}
                        <span style="color: red;">
                            <?php
                            DUP_PRO_U::esc_html_e("The home path of your WordPress installation is '/'.
                                Having '/' as your home path makes it impossible to run a search-and-replace on the database while installing the package.
                                You can still restore the package on the same server or install it on a different server and manually replace paths in the database.");
                            ?>
                        </span>
                    {{/if}}
                    ...
                    {{#if ARC.Status.PackageIsNotImportable}}
                        ...
                        <p>
                            ...
                            {{#if ARC.Status.IsDBOnly}}
                                <?php DUP_PRO_U::esc_attr_e("Database only packages can only be installed via the installer.php file. The Drag and Drop interface only processes packages that have all WordPress core directories and all database tables."); ?>
                            {{else}}
...
<div id="dup-scan-db">
    <div class="scan-item">
        ...
        <div class="info">
            ...
            <?php
                printf(
                    DUP_PRO_U::__('Total size and row count are approximate values.  The thresholds that trigger warnings are <i>%1$s OR %2$s records</i> toal for the entire database.  '
                ...
                printf(
                    DUP_PRO_U::__('The notices for tables are <i>%1$s, %2$s records or names with upper-case characters</i>.  Individual tables will not trigger '
            ...
            <script id="hb-showcreatefunc-result" type="text/x-handlebars-template">
                <div class="container">
                    <div class="data">
                        ...
                        <?php DUP_PRO_U::esc_html_e("The database user for this WordPress site has sufficient permissions to write stored procedures and functions to the sql file of the archive. [The commands SHOW CREATE PROCEDURE/FUNCTION will work.]"); ?>
                        {{else}}
                        <span style="color: red;">
                            <?php
